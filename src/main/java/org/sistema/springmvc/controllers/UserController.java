...
public class UserController {
	...
	public String deleteall() {
		logger.info("Product detail /deleteall");
	 ...
	 * Delete the specific user
	 */
	@RequestMapping(method = RequestMethod.GET, value = { "/users/order/{ord}" })
	public String orderUsers(@PathVariable(value = "ord") String order, Map<String, Object> model){
		logger.info("Product showUsers ordered ");
		List<User> users = userDAO.AllOrdered(order);
		model.put("users", users);
		model.put("searchUser", new User());
		return "user/users";
	...
	public String searchUsers(User user, Model model) {
		List<User> users = userDAO.searchUsers(user.getLogin());
